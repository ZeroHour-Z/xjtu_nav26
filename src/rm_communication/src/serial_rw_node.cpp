            ...
            if (header == kNavHeader) {
                ...
                RCLCPP_INFO_THROTTLE(
                    ...
                    *this->get_clock(),
                    ...
                    "[nav] RX color:%d state:%d motion:%d posture:%d posture_reinforce:%d remain_time:%d power_limit:%d cooling:%d region:%d hp:%d bullet:%d enemy:(%.2f,%.2f) target:(%.2f,%.2f)",
                    (int)n_data.color,
                    (int)n_data.eSentryState,
                    (int)n_data.motion_allowed,
                    (int)n_data.posture,
                    (int)n_data.posture_reinforce,
                    (int)n_data.stage_remain_time,
                    (int)n_data.power_limit,
                    (int)n_data.cooling_value,
